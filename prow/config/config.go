...
func (c *Config) validateComponentConfig() error {
	...
	if c.Deck.RerunAuthConfig != nil {
		logrus.Warning("rerun_auth_config will be deprecated on March 2020, and it will be replaced with rerun_auth_configs['*'].")
...
func validateJobBase(v JobBase, jobType prowapi.ProwJobType, podNamespace string) error {
	...
	if err := validateRerunAuthConfig(v.RerunAuthConfig); err != nil {
		return err
	}
...
func validateRerunAuthConfig(rac *prowapi.RerunAuthConfig) error {
	if rac == nil {
		return nil
	}
